...
def mask(targets, out):
    # targets: B x (T-1) 
    # out    : B x (T-1) x d_model
    # return : 
    #          (#NonZero, ), (#NonZero, d_model)
    mask = (targets != 1)
...
class Attention(nn.Module):
    ...
    def __init__(self, d_key, drop_ratio, causal):
        """ if causal == True: means mask for causal
        """
        super().__init__()
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model, d_hidden=2048,
        ...
        super().__init__()
        self.encoder = Encoder(d_model, d_hidden, n_layers,
                               n_heads, drop_ratio)
